...
async def open_creator_home(headless=False, persistent=True):
    ...
    if not persistent and not os.path.exists(AUTH_FILE):
        raise FileNotFoundError("未找到登录状态文件，请先运行 test/integration/browser_session_control.py --target creator --mode login 手动登录。")
...
async def open_xhs_home(headless=False, persistent=True):
    ...
    if not persistent and not os.path.exists(XHS_WEB_AUTH_FILE):
        raise FileNotFoundError("未找到小红书主页登录状态文件，请先运行 test/integration/browser_session_control.py --target web --mode login 手动登录。")
    p = await async_playwright().start()
